...
public class ShareTargetTests : IAsyncLifetime
{
    ...
    {
        _testDbPath = Path.Combine(Path.GetTempPath(), $"openchat_share_test_{Guid.NewGuid()}.db");
        _storageService = new StorageService(_testDbPath, new MockSecureStorage());
        _testRootDir = Path.Combine(Path.GetTempPath(), $"openchat_share_root_{Guid.NewGuid()}");
    }
    ...
    private string SetUpProfileDb(string pubKeyHex)
    {
        ...
        return Path.Combine(profileDir, "openchat.db");
    }
